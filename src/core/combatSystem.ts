...
export class CombatSystem {
   ...
   */
  public async executeFullCombat(
    p1: EffectiveStats,
    p2: EffectiveStats,
    battleId: string,
  ): Promise<CombatState> {
    ...
    while (currentP1.hp > 0 && currentP2.hp > 0 && state.turnCount < 50) {
      ...
      const [first, second] =
        currentP1.speed >= currentP2.speed
          ? [currentP1, currentP2]
          : [currentP2, currentP1];

      // First attacker
      const turn1 = this.executeTurn(
        first as EffectiveStats,
        second as EffectiveStats,
      );
  ...
  private runTurn(
    ...
    if (attacker.mana < runtimeSkill.manaCost) {
      state.logs.push(
        `${attacker.name} tried to use skill [${runtimeSkill.id}] but failed (Not enough Mana).`,
      );
    ...
    state.logs.push(
      `${attacker.name} uses skill ${runtimeSkill.id} (${runtimeSkill.manaCost} MP)`,
    );
  ...
  private applySkill(
    ...
    if (skill.damage > 0) {
      ...
      state.logs.push(
        `${defender.name} takes ${finalDamage} damage. -> HP: ${defender.hp}/${defender.maxHP}`,
      );
    }
    ...
    if (skill.healing > 0) {
      ...
      state.logs.push(
        `${attacker.name} heals for ${skill.healing} HP. -> HP: ${attacker.hp}/${attacker.maxHP}`,
      );
    }
