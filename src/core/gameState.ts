...
const TRANSITIONS: Record<GamePhase, GamePhase[]> = {
    ...
    [GamePhase.EVENT]: [GamePhase.COMBAT, GamePhase.DIALOGUE, GamePhase.LOOT, GamePhase.EXPLORING],
...
export class GameStateManager {
    ...
    constructor(playerId: string, characterId: string, worldSeed: number) {
        this.state = {
            ...
            playerId, characterId, worldSeed,
            regionIndex: 0, mapId: null, eventId: null, combatId: null,
            ...
            effectiveStats: {
                id: characterId, name: 'Hero', level: 1,
                hp: 100, maxHP: 100, mana: 50, maxMana: 50,
                attack: 12, defense: 6, speed: 10, skillMain: 111111111,
                baseStats: { id: characterId, name: 'Hero', level: 1, hp: 100, maxHP: 100, mana: 50, maxMana: 50, attack: 12, defense: 6, speed: 10, skillMain: 111111111 },
                modifiers: { attackBonus: 0, defenseBonus: 0, speedBonus: 0, hpBonus: 0 }
            }
        };
    }

    public getState(): Readonly<GameStateData> { return this.state; }
    public getPhase(): GamePhase { return this.state.phase; }
    ...
    public getCharacterStats(equipmentMetadata: any[]): EffectiveStats {
        const equippedIds = Object.values(this.state.equipment).filter(id => !!id);
        const equippedData = equipmentMetadata.filter(e => equippedIds.includes(e.id) || equippedIds.includes(e.name)); // handles both id and name
    ...
    public useItem(item: any): string | null {
        ...
        if (statBonus.mana) {
            ...
            this.state.mana = Math.min(this.state.maxMana, this.state.mana + restored);
    ...
    public static deserialize(json: string): GameStateManager {
        ...
        const mgr = new GameStateManager(data.playerId, data.characterId, data.worldSeed);
