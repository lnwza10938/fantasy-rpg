...
export class SkillInterpreter {
    ...
    public interpret(skill: Skill, caster: CharacterStats): RuntimeSkill {
        ...
        } else if (skill.role === SkillRole.Buff || skill.role === SkillRole.Debuff) {
        ...
        if (skill.effect === EffectType.Soul || skill.effect === EffectType.MaxHPDamage) {
    ...
    private calculatePowerScale(scaling: ScalingSource, caster: CharacterStats): number {
        switch (scaling) {
            case ScalingSource.AttackPower: return caster.attack;
            case ScalingSource.DefensePower: return caster.defense;
            case ScalingSource.MaxHP: return Math.floor(caster.maxHP * 0.1);
            case ScalingSource.MaxMana: return Math.floor(caster.maxMana * 0.2);
            case ScalingSource.Speed: return caster.speed;
            case ScalingSource.Level: return caster.level * 3;
            default: return 10;
        }
    }
    ...
    private calculateRange(delivery: DeliveryShape): number {
        switch (delivery) {
            case DeliveryShape.DirectStrike: return 1;
            ...
            case DeliveryShape.Ring:
                return 2;
            case DeliveryShape.Aura: return 0; // Self-centered
            default: return 1;
        }
    }
    ...
    private calculateDurationTurns(duration: SkillDuration): number {
        switch (duration) {
            ...
            case SkillDuration.Short: return 1;
            case SkillDuration.Medium: return 2;
            case SkillDuration.Long: return 3;
            case SkillDuration.Periodic: return 3;
            case SkillDuration.Continuous: return 5;
            case SkillDuration.Conditional: return 10;
            default: return 0;
        }
    }
}
