...
export class ContentValidator {
  public validateMonster(data: any): ValidationResult {
    ...
    if (data.level != null && (data.level < 1 || data.level > 100))
      errors.push("level must be 1-100");
    if (data.base_hp != null && data.base_hp <= 0)
      errors.push("base_hp must be > 0");
    if (data.base_attack != null && data.base_attack < 0)
      errors.push("base_attack must be >= 0");
    if (data.base_defense != null && data.base_defense < 0)
      errors.push("base_defense must be >= 0");
    if (data.skill_id != null) {
      const s = String(data.skill_id);
      if (s.length !== 9 || isNaN(Number(s)))
        errors.push("skill_id must be exactly 9 digits");
    }
  ...
  public validateEquipment(data: any): ValidationResult {
    ...
    if (!data.slot || !validSlots.includes(data.slot))
  ...
  public validateMap(data: any): ValidationResult {
    ...
    if (!data.name) errors.push("name is required");
    if (
      data.danger_level != null &&
      (data.danger_level < 1 || data.danger_level > 10)
    )
      errors.push("danger_level must be 1-10");
    return { valid: errors.length === 0, errors };
  }
  ...
  public validateSpawnPoint(data: any): ValidationResult {
    ...
    if (!Array.isArray(data.monster_pool) || data.monster_pool.length === 0)
      errors.push("monster_pool must be a non-empty array");
    if (data.spawn_rate != null && (data.spawn_rate < 0 || data.spawn_rate > 1))
      errors.push("spawn_rate must be 0.0-1.0");
    return { valid: errors.length === 0, errors };
  }
