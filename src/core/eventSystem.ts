...
export class EventSystem {
     ...
     *   5% Rare Event
     */
    public async generateEvent(player: CharacterStats, region: Region): Promise<GameEvent> {
    ...
    private triggerLoreEvent(region: Region, instance: any): GameEvent {
        const lorePool = instance.lore.length > 0 ? instance.lore : [
            { title: 'The Silent Era', content: 'Centuries ago, the world fell silent for a day.' }
        ];
    ...
    private triggerNPCEncounter(region: Region, instance: any): GameEvent {
        ...
        const npc = allNPCs.length > 0 ? allNPCs[Math.floor(Math.random() * allNPCs.length)] : fallbackNPCs[0];
    ...
    private async triggerEnemyEncounter(player: CharacterStats, region: Region, instance: any): Promise<GameEvent> {
        const poolNames = region.enemyPool;
        const monsters = instance.monsterPool.filter((m: any) => poolNames.includes(m.name));

        if (monsters.length === 0) return this.triggerNothing(region);

        // Weighted selection
        const totalWeight = monsters.reduce((sum: number, m: any) => sum + (m.spawn_weight || 1.0), 0);
        ...
        const enemy = worldSystem.spawnEnemy(selectedMonster.name, Math.max(1, player.level + (Math.random() > 0.8 ? 1 : 0)));
        ...
        // Auto-resolve combat
        const result = await combatSystem.executeFullCombat(effPlayer, effEnemy, uuidv4());
