...
import { getMaps, getMonsters, getFactions, getLoreSnippets } from '../db/contentRepositories.js';
...
export class WorldSystem {
    ...
    public async generateWorld(seed: number): Promise<WorldInstance> {
        ...
        this.worldInstance = new WorldInstance(seed, regions, allMonsters, allFactions, allLore);
    ...
    public spawnEnemy(name: string, level: number): CharacterStats {
        ...
        const baseMonster = this.worldInstance.monsterPool.find(m => m.name === name);
        ...
        const baseDefense = baseMonster?.base_defense || (2 + Math.floor(level * 0.8));
        const baseSpeed = baseMonster?.speed || (5 + Math.floor(level / 2));
        const skillMain = baseMonster?.skill_id || rng.nextInt(100000000, 999999999);
