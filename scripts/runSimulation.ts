...
async function runSimulation() {
    ...
    for (const r of world.regions) {
        console.log(`   [${r.id}] ${r.name} ⚠️${r.dangerLevel} — ${r.enemyTypes.join(', ')}`);
    }

    // 2. Create Hero
    const hero: CharacterStats = {
        id: 'hero_001', name: 'Raen', level: 8,
        hp: 120, maxHP: 120, mana: 60, maxMana: 60,
        attack: 18, defense: 8, speed: 14,
        skillMain: 863016382
    };
    ...
    for (let i = 0; i < 3; i++) {
        ...
        if (event.type === EventType.EnemyEncounter && event.combatLogs) {
            ...
            if (hero.hp > 0 && event.enemy) {
                const legendText = legendSystem.formatLegend(hero.name, 'defeated', event.enemy.name, region.name);
